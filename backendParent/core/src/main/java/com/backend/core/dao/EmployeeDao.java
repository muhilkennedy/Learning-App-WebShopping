...
public class EmployeeDao {
	...
	public void createAdminUserForTenant(String tenantId) throws Exception {
		try (Connection con = dbUtil.getConnectionInstance()) {
			PreparedStatement stmt = con
					.prepareStatement("insert into employeeinfo (tenantid , fname, lname, emailid, mobile, password, designation, lastlogin) values (?, 'SUPERUSER', 'PRODUCTION', 'prodemail@email.com', '1234567890', ?, 'CUSTOMERSUPPORTADMIN', ?);");
