...
CREATE TABLE IF NOT EXISTS tenantdetails (tenantid varchar(50) NOT NULL UNIQUE, tenantdetailid int NOT NULL AUTO_INCREMENT PRIMARY KEY, tenantcontact varchar(10), tenantemail varchar(100), tenantbusinessemail varchar(100) NOT NULL, businessemailpassword varchar(2000) NOT NULL, tenantstreet varchar(100), tenantcity varchar(50), tenantpin varchar(10), tenantlogo MEDIUMBLOB, tenantfacebook varchar(100), tenanttwitter varchar(100), tenantinsta varchar(100), GSTIN varchar(50), FSSAI varchar(50), CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
...
CREATE TABLE IF NOT EXISTS orders (tenantid varchar(50) NOT NULL, orderid int NOT NULL AUTO_INCREMENT PRIMARY KEY, customerid int NOT NULL , orderdate double, status varchar(50) NOT NULL, subtotal DECIMAL(10,2), couponapplied BOOL DEFAULT FALSE, coupondiscount int, couponid int, employeeid int, paymentmodeid int NOT NULL, courierid varchar(100), courierservicename varchar(100), CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(customerid) REFERENCES customerinfo(customerid), CONSTRAINT FOREIGN KEY(paymentmodeid) REFERENCES paymentmode(paymentmodeid));
...
insert into customeraddress (tenantid, customerid, doornumber, street, city, pincode, state) values ('devTenant', 1, '13', 'Master street', 'vathi raid', '603104', 'TN');
