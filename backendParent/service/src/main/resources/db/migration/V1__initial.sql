...
CREATE TABLE IF NOT EXISTS orders (tenantid varchar(50) NOT NULL, orderid int NOT NULL AUTO_INCREMENT PRIMARY KEY, customerid int NOT NULL , orderdate double, status varchar(50) NOT NULL, subtotal DECIMAL(10,2), couponapplied BOOL DEFAULT FALSE, coupondiscount int, couponid int, employeeid int, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(customerid) REFERENCES customerinfo(customerid));
...
insert into customerinfo (tenantid, customerid, fname, lname, emailid, password, active, loginvia, lastlogin) values ('devTenant', 1, 'muhil', 'kennedy', 'do.not.reply.application.ordering@gmail.com', '$2a$05$FueeaV.hfKbv/m6kG6GM../gKdRPOoihCQF1WBjpAPZNDlGPOxSha', true, 'INTERNAL', 1604160497704);
