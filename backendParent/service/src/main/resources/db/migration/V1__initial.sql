...
CREATE TABLE IF NOT EXISTS product (tenantid varchar(50) NOT NULL, categoryid int, productid int NOT NULL AUTO_INCREMENT PRIMARY KEY, productname varchar(50) NOT NULL, brand varchar(50) NOT NULL, cost DECIMAL(12,2) NOT NULL, offer DECIMAL(4,2) DEFAULT 0, description varchar(300) NOT NULL, productcode varchar(50) UNIQUE, unitsinstock int DEFAULT 0, lastmodified DATETIME DEFAULT CURRENT_TIMESTAMP, lastmodifiedemployeeid int, active BOOL DEFAULT TRUE, isDeleted BOOL DEFAULT FALSE, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(categoryid) REFERENCES category(categoryid), CONSTRAINT FOREIGN KEY(lastmodifiedemployeeid) REFERENCES employeeinfo(employeeid));
