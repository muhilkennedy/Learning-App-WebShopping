...
CREATE TABLE IF NOT EXISTS tenantdetails (tenantid varchar(50) NOT NULL UNIQUE, tenantdetailid int NOT NULL AUTO_INCREMENT PRIMARY KEY, tenantcontact varchar(10), tenantemail varchar(100), tenantbusinessemail varchar(100), businessemailpassword varchar(2000), tenantstreet varchar(100), tenantcity varchar(50), tenantpin varchar(10), tenantlogo MEDIUMBLOB, tenantfacebook varchar(100), tenanttwitter varchar(100), tenantinsta varchar(100), gstin varchar(50), fssai varchar(50), tagline varchar(100), CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
...
CREATE TABLE IF NOT EXISTS product (tenantid varchar(50) NOT NULL, categoryid int, productid int NOT NULL AUTO_INCREMENT PRIMARY KEY, productname varchar(50) NOT NULL, brand varchar(50) NOT NULL, cost DECIMAL(12,2) NOT NULL, offer DECIMAL(4,2) DEFAULT 0, description varchar(300) NOT NULL, productcode varchar(50) UNIQUE, unitsinstock int DEFAULT 0, lastmodified double, lastmodifiedemployeeid int, active BOOL DEFAULT TRUE, isdeleted BOOL DEFAULT FALSE, productrating int DEFAULT 0, sellingcost DECIMAL(12,2) NOT NULL, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(categoryid) REFERENCES category(categoryid), CONSTRAINT FOREIGN KEY(lastmodifiedemployeeid) REFERENCES employeeinfo(employeeid));
...
CREATE TABLE IF NOT EXISTS productreview (tenantid varchar(50) NOT NULL, productreviewid int NOT NULL AUTO_INCREMENT PRIMARY KEY, customerid int NOT NULL, productid int NOT NULL, rating int NOT NULL, review varchar(500) NOT NULL, reviewheader varchar(100) NOT NULL, usefullcount int, reviewtime double, CONSTRAINT FOREIGN KEY(productid) REFERENCES product(productid), CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(customerid) REFERENCES customerinfo(customerid));
CREATE TABLE IF NOT EXISTS productreviewsync (review JSON);

/* Coupons related tables */
CREATE TABLE IF NOT EXISTS coupons (tenantid varchar(50) NOT NULL, couponid int NOT NULL AUTO_INCREMENT PRIMARY KEY, title varchar(100) not null, code varchar(64) NOT NULL UNIQUE, discount int, startdate double, enddate double, freeshipping BOOL DEFAULT FALSE, userusage int, active BOOL DEFAULT TRUE, isdeleted BOOL DEFAULT FALSE, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
...
CREATE TABLE IF NOT EXISTS orders (tenantid varchar(50) NOT NULL, orderid int NOT NULL AUTO_INCREMENT PRIMARY KEY, customerid int NOT NULL , orderdate double, status varchar(50) NOT NULL, subtotal DECIMAL(10,2), couponapplied BOOL DEFAULT FALSE, coupondiscount int, couponid int, employeeid int, paymentmodeid int NOT NULL, customeraddressid int NOT NULL, courierid varchar(100), courierservicename varchar(100), CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(customerid) REFERENCES customerinfo(customerid), CONSTRAINT FOREIGN KEY(paymentmodeid) REFERENCES paymentmode(paymentmodeid));
...
CREATE TABLE IF NOT EXISTS unassignedorders (tenantid varchar(50) NOT NULL, orderid int NOT NULL, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
...
/* Invoice tables*/
CREATE TABLE IF NOT EXISTS invoicetemplate (tenantid varchar(50) NOT NULL, invoiceid int NOT NULL AUTO_INCREMENT PRIMARY KEY, document MEDIUMBLOB, posdocument MEDIUMBLOB, active BOOL DEFAULT TRUE, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
