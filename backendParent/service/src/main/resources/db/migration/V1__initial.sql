...
/* Customer tables */
CREATE TABLE IF NOT EXISTS customer (tenantid varchar(50) NOT NULL, customerid int NOT NULL PRIMARY KEY, fname varchar(50) NOT NULL, lname varchar(50) NOT NULL, emailid varchar(100) NOT NULL UNIQUE, mobile varchar(15) UNIQUE, password varchar(100), active BOOL DEFAULT TRUE, lastlogin double, profilepic MEDIUMBLOB, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
...
CREATE TABLE IF NOT EXISTS product (tenantid varchar(50) NOT NULL, categoryid int, productid int NOT NULL AUTO_INCREMENT PRIMARY KEY, productname varchar(50) NOT NULL, brand varchar(50) NOT NULL, cost DECIMAL(12,2) NOT NULL, offer DECIMAL(4,2) DEFAULT 0, description varchar(300) NOT NULL, productcode varchar(50) UNIQUE, unitsinstock int DEFAULT 0, lastmodified double, lastmodifiedemployeeid int, active BOOL DEFAULT TRUE, isdeleted BOOL DEFAULT FALSE, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(categoryid) REFERENCES category(categoryid), CONSTRAINT FOREIGN KEY(lastmodifiedemployeeid) REFERENCES employeeinfo(employeeid));
...
CREATE TABLE IF NOT EXISTS orders (tenantid varchar(50) NOT NULL, orderid int NOT NULL AUTO_INCREMENT PRIMARY KEY,customerid int NOT NULL ,orderdate double, status varchar(50) NOT NULL, subtotal int NOT NULL, couponapplied BOOL DEFAULT FALSE, couponid int, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(customerid) REFERENCES customer(customerid));
