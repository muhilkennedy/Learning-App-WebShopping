...
CREATE TABLE IF NOT EXISTS orders (tenantid varchar(50) NOT NULL, orderid int NOT NULL AUTO_INCREMENT PRIMARY KEY, customerid int NOT NULL , orderdate double, status varchar(50) NOT NULL, subtotal DECIMAL(4,2), couponapplied BOOL DEFAULT FALSE, coupondiscount int, couponid int, employeeid int, CONSTRAINT FOREIGN KEY(tenantid) REFERENCES tenant(tenantid), CONSTRAINT FOREIGN KEY(customerid) REFERENCES customerinfo(customerid));
