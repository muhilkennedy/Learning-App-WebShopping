...
CREATE TABLE IF NOT EXISTS dashboardreport (tenantid varchar(50) NOT NULL UNIQUE, id int NOT NULL AUTO_INCREMENT PRIMARY KEY, totalemail int, totalsms int, emailtoday int, smstoday int, totalpos int, totalonline int, postoday int, onlinetoday int, totalCustomers int, FOREIGN KEY(tenantid) REFERENCES tenant(tenantid));
